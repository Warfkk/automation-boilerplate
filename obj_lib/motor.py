...
class Motor:
    ...
    def _tia_db_multiple_plc(self):
        for plc in self.plc_set:
            ...
            data += self.gen.multiple(
                self.ol, self.cf, self.rl, "TIA_DB_Var", plc_name=plc
            )
            data += self.gen.single(self.cf, self.rl, "TIA_DB_Begin")
            data += self.gen.multiple(
                self.ol, self.cf, self.rl, "TIA_DB_Parameters", plc_name=plc
            )
            data += self.gen.single(self.cf, self.rl, "TIA_DB_Footer")
    ...
    def _tia_symbol(self):
        data = self.gen.multiple_config(self.ol, self.cp, self.rl, "TIA_Symbol")
